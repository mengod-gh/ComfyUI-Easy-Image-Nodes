...
| `exif_enabled` | BOOLEAN, optional | Defaults to `True`. If `True`, write PNG metadata as before. If `False`, save the image without prompt, workflow, or parameter metadata. |
| `positive` | STRING, optional | Positive prompt text to store in PNG `parameters` metadata when `exif_enabled` is `True`. |
| `negative` | STRING, optional | Negative prompt text to store in PNG `parameters` metadata when `exif_enabled` is `True`. |
...
| `exif_enabled` | BOOLEAN, optional | 기본값은 `True`입니다. `True`이면 기존처럼 PNG 메타데이터를 저장합니다. `False`이면 프롬프트, 워크플로우, parameter 메타데이터 없이 이미지만 저장합니다. |
| `positive` | STRING, optional | `exif_enabled`가 `True`일 때 PNG `parameters` 메타데이터에 저장할 positive prompt입니다. |
| `negative` | STRING, optional | `exif_enabled`가 `True`일 때 PNG `parameters` 메타데이터에 저장할 negative prompt입니다. |
...
| `exif_enabled` | BOOLEAN, optional | デフォルトは `True` です。`True` の場合は従来どおり PNG メタデータを書き込みます。`False` の場合は prompt、workflow、parameter メタデータなしで画像だけを保存します。 |
| `positive` | STRING, optional | `exif_enabled` が `True` の場合に PNG `parameters` メタデータへ保存する positive prompt です。 |
| `negative` | STRING, optional | `exif_enabled` が `True` の場合に PNG `parameters` メタデータへ保存する negative prompt です。 |
...
| `exif_enabled` | BOOLEAN, optional | 默认值为 `True`。如果为 `True`，按原有方式写入 PNG 元数据。如果为 `False`，保存不含 prompt、workflow 或 parameter 元数据的纯图片。 |
| `positive` | STRING, optional | 当 `exif_enabled` 为 `True` 时，写入 PNG `parameters` 元数据的 positive prompt。 |
| `negative` | STRING, optional | 当 `exif_enabled` 为 `True` 时，写入 PNG `parameters` 元数据的 negative prompt。 |
