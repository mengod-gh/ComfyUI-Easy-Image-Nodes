...
| `exif_enabled` | BOOLEAN, optional | If `True`, write PNG metadata as before. If `False`, save the image without prompt, workflow, or parameter metadata. |
...
| `exif_enabled` | BOOLEAN, optional | `True`이면 기존처럼 PNG 메타데이터를 저장합니다. `False`이면 프롬프트, 워크플로우, parameter 메타데이터 없이 이미지만 저장합니다. |
...
| `exif_enabled` | BOOLEAN, optional | `True` の場合は従来どおり PNG メタデータを書き込みます。`False` の場合は prompt、workflow、parameter メタデータなしで画像だけを保存します。 |
...
| `exif_enabled` | BOOLEAN, optional | 如果为 `True`，按原有方式写入 PNG 元数据。如果为 `False`，保存不含 prompt、workflow 或 parameter 元数据的纯图片。 |
